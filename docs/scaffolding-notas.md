...
**Nota para cuando crezca esta superficie** (aportada por la sesión par `otrarondamas-fb` al revisar este incremento, no es un gap de ahora): los 3 endpoints de `tienda.controller.ts` son la primera vez que el backend expone rutas sin autenticación más allá de `auth/login` — correcto por diseño (es la tienda pública), pero cuando se sume el checkout con pago real (Fase 5) vale la pena revisar rate-limiting y endurecer la validación de input ahí, antes de que esa superficie sin auth crezca más.
...
## 25. Tienda Online, Fase 4 — bandeja de pedidos en el panel (22/09/2026)

Contrapartida del vendedor: sin esto, un pedido de tienda online quedaba `RECIBIDO` sin que nadie del panel lo viera (confirmado como riesgo alto en el roadmap tras la Fase 3). Solo dos transiciones implementadas — decisión explícita del dueño confirmada antes de escribir código: `RECIBIDO → CONFIRMADO` (descuenta stock) o `RECIBIDO → CANCELADO` (no toca stock). El resto de `EstadoPedido` (`EN_PREPARACION`, `LISTO`, `ASIGNADO`, `EN_CAMINO`, `ENTREGADO`, `PARCIALMENTE_ENTREGADO`, `ENTREGA_FALLIDA`) queda para cuando se implemente RF-13 (Entregas) — no se construyó esa máquina de estados sin el módulo detrás.

**Refactor central de este incremento**: `VentasService.descontarStock()` (antes `private`) se movió a `InventarioService.descontarStock()`, parametrizando `motivo`/`referenciaId`/`usuarioId` — es el ÚNICO mecanismo de descuento de stock del sistema, usado ahora tanto por `VentasService` (venta presencial) como por `PedidosService` (confirmar un pedido online). Confirmar un pedido genera un `MovimientoStock` con `motivo: 'Venta'` (no un motivo nuevo — es una venta real, solo que se originó por el canal web) y `referenciaId` apuntando al `Pedido`, no a una `Venta` — el pedido de tienda online nunca crea una fila en `Venta`/`VentaItem`, esos modelos siguen exclusivos del POS presencial (decisión ya tomada en la Fase 1). `VentasModule` ahora importa `InventarioModule` para inyectar el service.

**Permiso nuevo**: `pedidos.gestionar` (seed.ts) — no se reusó `ventas.crear` (gestionar qué entra por la tienda es una autorización de negocio distinta de vender presencialmente, mismo criterio que `compras.gestionar`). A diferencia de Compras/Caja (donde el `GET` queda abierto a cualquier usuario logueado), acá **todo** — incluidos los `GET` — requiere el permiso: un `Pedido` expone datos personales del cliente (nombre, email, teléfono), así que ver la bandeja en sí ya es la operación sensible, no solo confirmar/cancelar. Decisión confirmada explícitamente antes de escribir el controller. Corrido `npm run prisma:sync-permisos` después del seed para propagar el permiso nuevo a los usuarios owner existentes — funcionó exactamente para el caso que ese script fue diseñado a resolver (sección 24 de este documento).

**Backend** (`apps/api/src/pedidos/`, módulo nuevo, privado):
- `GET /pedidos?estado=` — lista pedidos con `cliente` (nombre/email/teléfono) y `pedidoItems` con el nombre del producto resuelto.
- `GET /pedidos/:id` — detalle completo.
- `PATCH /pedidos/:id/estado` — solo acepta `CONFIRMADO`/`CANCELADO` (`class-validator` `@IsIn`), rechaza con 400 si el pedido no está en `RECIBIDO` (evita doble confirmación/cancelación). Confirmar corre en una `$transaction`: si el stock no alcanza para algún ítem, la confirmación entera se revierte y el pedido queda `RECIBIDO`, nunca "a medio confirmar".

**Frontend** (`PedidosPage.tsx` en `pos-admin`, reemplaza el placeholder "Pedidos - En desarrollo" que ya existía en `App.tsx`/`Sidebar.tsx`): dos secciones — pendientes de confirmar (con botones Confirmar/Cancelar) y ya gestionados (solo lectura). No oculta las acciones según permiso (mismo criterio que `CajaPage`/`InventarioPage`/`ComprasPage`) — el backend rechaza con 403 si corresponde.
...
## 26. Corte de dominios — completado y verificado en producción (22/09/2026)

Preparación de código/infra (Dockerfile, nginx.conf, compose, vhosts) hecha primero sin poder activarse — `admin.otrarondamas.wapsell.com` no tenía DNS todavía (`nslookup` sin resultado). El dueño agregó el registro A en el proveedor de `wapsell.com`; una vez confirmada la propagación (`nslookup` → `89.167.96.239`), se ejecutó el corte completo el mismo día.

**Resultado final, verificado en producción real:**
- `https://otrarondamas.wapsell.com` → tienda pública (`tienda-online`, puerto 3040).
- `https://admin.otrarondamas.wapsell.com` → panel interno (`pos-admin`, puerto 3030), certificado propio emitido y con renovación automática de certbot.
- `https://api.otrarondamas.wapsell.com` → sin cambios.
- Antes de preguntarle al dueño si había alguien usando el panel en ese momento (nadie, confirmado) — el corte reemplaza en el momento lo que sirve `otrarondamas.wapsell.com`, no es reversible sin otro deploy.

**Pasos ejecutados, en orden:**
1. Vhost temporal solo-HTTP para `admin.*` (sin bloque `443 ssl`, que hubiera fallado `nginx -t` por certificados inexistentes) — necesario porque `certbot --webroot` requiere que ALGO responda en el puerto 80 de ese `server_name` para servir el challenge; sin ningún vhost activo, el intento inicial de certbot falló con 404 antes de emitir nada.
2. `certbot certonly --webroot -d admin.otrarondamas.wapsell.com --register-unsafely-without-email` (reusa la cuenta de certbot ya registrada en el VPS, sin pedir un email nuevo) — certificado emitido en el primer intento tras el paso 1.
3. Reemplazo del vhost temporal por el `.conf` completo (con `443 ssl`), `nginx -t && reload`.
4. `git pull` en `/infra/projects/0010-otrarondamas` para traer el `docker-compose.prod.yml`/vhosts actualizados de este repo.
5. Migración de `.env` real del VPS: `VITE_API_URL` → `VITE_API_URL_ADMIN`/`VITE_API_URL_TIENDA` (mismo valor de API para ambas), `CORS_ORIGINS` con ambos dominios, `FRONTEND_URL` → `admin.*`. Backup del `.env` viejo guardado junto al real antes de tocarlo (`.env.bak-corte-dominios-<fecha>`).
6. `docker compose build pos-admin tienda-online` + `up -d api pos-admin tienda-online`.

**Gap real encontrado durante la ejecución, no anticipado en la preparación**: el `git pull` en `/infra/projects/0010-otrarondamas` actualiza el checkout del repo, pero **nginx lee de `/etc/nginx/sites-available/`+`sites-enabled/`, que es una copia manual, no un symlink al repo**. El primer intento de corte dejó ambos dominios sirviendo `pos-admin` (mismo hash de bundle en los dos) porque el `proxy_pass` viejo (puerto 3030) seguía activo en `/etc/nginx/sites-available/otrarondamas.wapsell.com.conf` pese al pull. Detectado comparando el hash del bundle JS servido por cada dominio contra el hash real de cada build (no alcanza con verificar HTTP 200 — ambos dominios respondían 200 sirviendo la app equivocada). Corregido copiando explícitamente `infra/nginx/*.conf` del repo a `/etc/nginx/sites-available/` antes de cada `reload` — **este paso manual de copia es necesario en cada cambio de vhost futuro en este VPS**, el `git pull` por sí solo no lo actualiza.

**Verificado end-to-end en producción tras el corte**: cada dominio sirve el bundle JS correcto (hash comparado explícitamente, no solo HTTP 200); CORS acepta ambos orígenes nuevos; login real funciona desde `admin.*`; un pedido creado desde `otrarondamas.wapsell.com` (tienda) aparece en la bandeja y se confirma correctamente desde `admin.otrarondamas.wapsell.com` (panel) — flujo completo de punta a punta con los dominios finales. TLS válido en `admin.*`, redirect HTTP→HTTPS funcionando. Datos y stock de prueba revertidos después de validar.

## 27. Tienda Online, Fase 6 — descuento por producto (22/09/2026)

D-01/D-02 del SDD siguen sin definirse en general (reglas de precio mayorista, descuentos por cantidad/cliente), pero el propio SDD acota el alcance de esta fase de forma explícita: *"la tienda online utilizará un precio base único, con posibilidad de aplicar descuentos"* (RF-04) — no pide lógica mayorista ni reglas de cantidad para la tienda, solo un descuento sobre ese precio único. Con eso, la fase se resolvió sin esperar D-01/D-02 completos: descuento único, global y por porcentaje sobre `precioMinorista`, confirmado con el dueño antes de tocar el schema (alcance del descuento, permiso que lo protege, rango 0-100 sin tope de negocio propio).

**Backend:**
- `Producto.descuentoPorcentaje` (`Decimal? @db.Decimal(5,2)`, migración `20260922080442_tienda_online_fase6_descuento_producto`) — `null` o `0` equivalen a "sin descuento".
- `CreateProductoDto`/`UpdateProductoDto` (heredado vía `PartialType`) aceptan el campo, validado `@Min(0) @Max(100)`. Reusa el permiso `productos.gestionar` ya existente — no se creó uno nuevo, el descuento es un atributo más del producto, mismo criterio que precio o stock mínimo.
- `TiendaService.precioConDescuento()` — helper privado, único lugar donde se calcula el precio final. Usado tanto en `catalogo()` (expone `precio` ya con descuento, `precioSinDescuento` solo si hay descuento activo, y `descuentoPorcentaje`) como en `crearPedido()` (el `precioUnitario` congelado en el `PedidoItem` YA incluye el descuento — es el precio real cobrado, no el bruto). Redondeado a 2 decimales con `Prisma.Decimal`, nunca aritmética de punto flotante nativa sobre dinero.
- `catalogo.controller.ts`/`ventas.service.ts` sin tocar — el descuento es exclusivo de la tienda pública (RF-06), no afecta ventas presenciales ni el catálogo interno (que sigue mostrando `precioMinorista` bruto, correcto: el vendedor necesita ver el precio de lista, el descuento es una política de canal).

**Frontend:**
- `PreciosPage.tsx` (pos-admin, ruta `/precios`) — pantalla mínima nueva, no un CRUD de catálogo completo (eso es otro alcance, no se construyó). Busca un producto (reusa el mismo patrón de `BuscadorProductos.tsx`) y edita solo el `%` de descuento inline, vía el `PATCH /catalogo/productos/:id` que ya existía sin UI. Decisión explícita del dueño: sin esta pantalla, cargar un descuento requería tocar la API a mano.
- `CatalogoPage.tsx` (tienda-online) — muestra el precio original tachado + el precio con descuento + el `%`, cuando corresponde; sin cambios cuando no hay descuento.
...
## 30. Fidelización, Fase 3 — niveles de fidelidad por historial (22/09/2026)

Umbrales confirmados con el dueño antes de tocar código (mismos valores que la propuesta ilustrativa del roadmap, ahora reales): **Nuevo** 0-2 compras, **Frecuente** 3-9, **VIP** 10+. Configurables por env var (`FIDELIZACION_UMBRAL_FRECUENTE`, `FIDELIZACION_UMBRAL_VIP`, default 3 y 10) — mismo patrón que `INVENTARIO_ALERTA_VENCIMIENTO_DIAS`, no hardcodeados en código para poder ajustarlos sin una migración.

- `ClientesService.calcularNivel(empresaId, clienteId)` — cuenta `Venta` (`estado: 'CONFIRMADA'`, hoy el único estado que existe — no hay anulación implementada) + `Pedido` (`estado: 'CONFIRMADO'`) asociados al cliente, vía `count()` (soportado por `empresaScopeExtension`, sin necesitar `groupBy`). Se calcula al vuelo en cada request — **no se persiste un campo `nivel` desnormalizado** que pueda desactualizarse, mismo criterio ya aplicado en `InventarioService.stockConsolidado()`.
- `ClientesService.listar()`/`obtener()` ahora devuelven el `nivel` calculado junto a los datos del cliente. `crear()`/`actualizar()` (Fase 1) **no** lo recalculan — devuelven el registro recién escrito tal cual, más rápido y consistente con que esas operaciones no tocan el historial de compras.
- `shared-types`: `Cliente.nivel?: NivelFidelidad` — opcional a propósito, porque `POST`/`PATCH /clientes` no lo traen (ver arriba). `ClientesPage.tsx` lo resuelve con criterio: un cliente recién creado siempre es `NUEVO` (0 compras, sin ambigüedad posible); uno recién editado conserva el nivel que ya tenía en la lista local, sin necesidad de otro request (editar el teléfono no cambia el historial de compras de nadie).
